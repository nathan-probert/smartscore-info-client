...
def test_get_stats_on_initialization(mock_players_api_response):
    with patch("schemas.player_info.exponential_backoff_request") as mock_request:
...
@patch("schemas.player_info.exponential_backoff_request")
def test_manual_stat_update(mock_request, player_info_with_stats):
...
@patch("schemas.player_info.exponential_backoff_request")
def test_get_gpg(mock_request, mock_players_api_response):
    mock_request.return_value = mock_players_api_response
...
@patch("schemas.player_info.exponential_backoff_request")
def test_get_hgpg(mock_request, mock_players_api_response):
    mock_request.return_value = mock_players_api_response
...
@patch("schemas.player_info.exponential_backoff_request")
def test_get_five_gpg(mock_request, mock_players_api_response):
    mock_request.return_value = mock_players_api_response
